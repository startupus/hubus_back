...
Write-Host "=== Testing Referral Links System ===" -ForegroundColor Green
...
# Step 3: Register user 2 with referral LINK (not code)
Write-Host "`n3. Registering user 2 with referral LINK..." -ForegroundColor Yellow
$user2Data = @{
    name = "User 2 Company"
    email = $user2Email
    password = "password123"
...
try {
    ...
    Write-Host "User 2 registered with referral link successfully" -ForegroundColor Green
...
if ($user2Response.company.referredBy -eq $user1Id) {
    ...
    Write-Host "  User 2 was referred by User 1 via referral link" -ForegroundColor Cyan
} else {
...
# Step 5: Check referral stats (should work now)
...
try {
    ...
    if ($referralStats.totalUses -eq 1 -and $referralStats.totalReferrals -eq 1) {
        Write-Host "Referral stats are correct" -ForegroundColor Green
    } else {
        Write-Host "Referral stats may be incorrect" -ForegroundColor Yellow
    }
} catch {
    Write-Host "Error getting referral stats: $($_.Exception.Message)" -ForegroundColor Red
}

# Step 6: Test referral link validation
Write-Host "`n6. Testing referral link validation..." -ForegroundColor Yellow
try {
    ...
    Write-Host "Referral link validation test" -ForegroundColor Green
    Write-Host "  Is Valid: $($validationResponse.isValid)" -ForegroundColor Cyan
    Write-Host "  Message: $($validationResponse.message)" -ForegroundColor Cyan
} catch {
    Write-Host "Error validating referral link: $($_.Exception.Message)" -ForegroundColor Red
}

Write-Host "`n=== Referral Links System Test Completed Successfully ===" -ForegroundColor Green
...
Write-Host "  Referral LINK usage works" -ForegroundColor Green
Write-Host "  Referral relationship is established via link" -ForegroundColor Green
Write-Host "  Referral stats are tracked correctly" -ForegroundColor Green
Write-Host "  Referral link validation works" -ForegroundColor Green
Write-Host "`nThe referral links system is working correctly!" -ForegroundColor Green
