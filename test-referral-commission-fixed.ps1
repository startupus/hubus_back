...
Write-Host "=== TESTING REFERRAL COMMISSION SYSTEM (FIXED) ===" -ForegroundColor Green
...
try {
    # Check inviter balance using correct endpoint
    ...
    # Check referral balance using correct endpoint
...
try {
    ...
    Write-Host "  Amount spent: $($spendResponse.balance.balance)" -ForegroundColor Cyan
...
try {
    # Use the correct endpoint for referral stats
    $billingReferralStats = Invoke-RestMethod -Uri "http://localhost:3004/billing/referral/stats" -Method GET -Headers @{Authorization = "Bearer $inviterToken"}
