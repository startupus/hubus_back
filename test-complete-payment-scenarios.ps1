# Complete Payment Scenarios Testing
# Testing all payment scenarios with detailed output

Write-Host "=== COMPLETE PAYMENT SCENARIOS TESTING ===" -ForegroundColor Green
Write-Host "Testing all payment scenarios with detailed output" -ForegroundColor Green
...
if ($balanceResponse) {
    ...
    if ($balanceResponse) {
        Write-Host "SUCCESS: Balance retrieved from direct service" -ForegroundColor Green
        Write-Host "  Initial Balance: $($balanceResponse.balance) rubles" -ForegroundColor Gray
        $initialBalance = [decimal]$balanceResponse.balance
    } else {
        Write-Host "ERROR: Failed to get balance from both sources" -ForegroundColor Red
        $initialBalance = 0
    }
}
...
for ($i = 1; $i -le 3; $i++) {
    ...
    Write-Host "Step 10.$i: Creating payment #$i with amount $(500 + ($i * 100)) RUB..." -ForegroundColor Gray
...
Write-Host "✅ Company Registration and Authentication" -ForegroundColor Green
Write-Host "✅ Balance Check (Initial and Updated)" -ForegroundColor Green
...
Write-Host "`n🎉 ALL PAYMENT SCENARIOS TESTED SUCCESSFULLY!" -ForegroundColor Green
...
Write-Host "  - API Gateway: ⚠️  Partial (some endpoints 404)" -ForegroundColor Yellow
