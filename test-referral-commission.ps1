...
Write-Host "=== TESTING REFERRAL COMMISSION SYSTEM ===" -ForegroundColor Green
Write-Host "Testing referral registration and commission payments..." -ForegroundColor Yellow
...
try {
    ...
    $inviterToken = $inviterResponse.accessToken
...
# ========================================
Write-Host "`nSTEP 4: Check initial balances" -ForegroundColor Magenta

try {
    # Check inviter balance
    $inviterBalance = Invoke-RestMethod -Uri "http://localhost:3004/billing/balance/$inviterId" -Method GET
    Write-Host "Inviter initial balance: $($inviterBalance.balance)" -ForegroundColor Cyan
    
    # Check referral balance
    $referralBalance = Invoke-RestMethod -Uri "http://localhost:3004/billing/balance/$referralId" -Method GET
    Write-Host "Referral initial balance: $($referralBalance.balance)" -ForegroundColor Cyan
} catch {
    Write-Host "ERROR: Error checking balances: $($_.Exception.Message)" -ForegroundColor Red
}

# ========================================
# STEP 5: Simulate AI request by referral (spend money)
# ========================================
Write-Host "`nSTEP 5: Simulate AI request by referral (spend money)" -ForegroundColor Magenta
...
try {
    $spendResponse = Invoke-RestMethod -Uri "http://localhost:3004/billing/update-balance/$referralId" -Method POST -Body $spendData -ContentType "application/json"
...
# ========================================
# STEP 6: Check balances after transaction
# ========================================
Write-Host "`nSTEP 6: Check balances after transaction" -ForegroundColor Magenta

try {
    # Check inviter balance (should have received commission)
    $inviterBalanceAfter = Invoke-RestMethod -Uri "http://localhost:3004/billing/balance/$inviterId" -Method GET
    Write-Host "Inviter balance after: $($inviterBalanceAfter.balance)" -ForegroundColor Cyan
    
    # Check referral balance (should be lower)
    $referralBalanceAfter = Invoke-RestMethod -Uri "http://localhost:3004/billing/balance/$referralId" -Method GET
    Write-Host "Referral balance after: $($referralBalanceAfter.balance)" -ForegroundColor Cyan
    
    # Calculate commission
    $commissionReceived = $inviterBalanceAfter.balance - $inviterBalance.balance
...
# ========================================
# STEP 7: Check referral statistics
# ========================================
Write-Host "`nSTEP 7: Check referral statistics" -ForegroundColor Magenta
...
# ========================================
# STEP 8: Check billing referral statistics
# ========================================
Write-Host "`nSTEP 8: Check billing referral statistics" -ForegroundColor Magenta

try {
    $billingReferralStats = Invoke-RestMethod -Uri "http://localhost:3004/billing/referral/stats/$inviterId" -Method GET
    
    Write-Host "SUCCESS: Billing referral statistics:" -ForegroundColor Green
    Write-Host "  Total referrals: $($billingReferralStats.totalReferrals)" -ForegroundColor Cyan
    Write-Host "  Total earnings: $($billingReferralStats.totalEarnings)" -ForegroundColor Cyan
    Write-Host "  Total transactions: $($billingReferralStats.totalTransactions)" -ForegroundColor Cyan
} catch {
    Write-Host "ERROR: Error getting billing referral stats: $($_.Exception.Message)" -ForegroundColor Red
}

# ========================================
# FINAL RESULT
# ========================================
Write-Host "`nREFERRAL COMMISSION TEST RESULT:" -ForegroundColor Green
